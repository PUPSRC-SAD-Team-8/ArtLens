                    ...
                    <div class="modal-content1">
                        ...
                        <div id="formContent">
                            ...
                            <div class="form-floating mb-3">
                                ...
                                <div id="dateTimeError" class="invalid-feedback" style="display: none; color: red; font-size: smaller;"></div> <!-- Error message container -->
                            </div>
...
<script src="assets/js/logformvalidationinput.js"></script>

<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script>
    $(document).ready(function() {
        // Initialize modal with form content visible and "Form" button active
        $('#formContent').show();
        $('#statusContent').hide();
        $('.form').addClass('active'); // Set "Form" button as active initially

        // Close modal and reset form fields
        $('.close2').click(function() {
            $('#bookingForm')[0].reset(); // Reset booking form
            $('#statusForm')[0].reset(); // Reset status form
            $('#formContent').show(); // Show form content
            $('#statusContent').hide(); // Hide status content
            $('#statusMessage').empty(); // Clear status message content
            $('#contact_email').val(''); // Reset contact_email input field
            $('.form').addClass('active'); // Set "Form" button as active
            $('.status').removeClass('active'); // Remove "Status" button active state

            // Set toggle buttons to default state (Form active)
            $('.form').css('background-color', '#4169E1').css('color', 'white');
            $('.status').css('background-color', 'white').css('color', 'black');

            // Show default status image when status content is emptied
            $('#noInfoImage').show();
        });
        ...
        $('.form').click(function() {
            $('#formContent').show();
            $('#statusContent').hide();
            $('#statusMessage').empty(); // Clear status message content when switching to form
            $('#noInfoImage').hide(); // Hide default status image
            $(this).addClass('active'); // Set clicked button as active
            $('.status').removeClass('active'); // Remove active state from other button
        });
        ...
        $('.status').click(function() {
            $('#formContent').hide();
            $('#statusContent').show();
            $('#statusMessage').empty(); // Clear status message content when switching to status
            $('#contact_email').val(''); // Reset contact_email input field
            $('#noInfoImage').show(); // Hide default status image
            $(this).addClass('active'); // Set clicked button as active
            $('.form').removeClass('active'); // Remove active state from other button
        });
        ...
        $('#bookingForm').submit(function(event) {
            ...
            $('#alertMessage').removeClass('d-none'); // Show success message
        });
        ...
        $('.btn3').click(function() {
            ...
            // Example: If status is empty, show default image
            if (statusIsEmpty()) {
                $('#noInfoImage').show();
            } else {
                $('#noInfoImage').hide();
        ...
        // Function to check if status content is empty (example function)
        function statusIsEmpty() {
            // Implement your logic to check if status content is empty
            return $('#statusMessage').html().trim() === '';
        }
    });
</script>
